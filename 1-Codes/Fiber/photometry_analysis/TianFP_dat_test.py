...
def import_CMOS_fear_data(file_name, animal_id):
    ...
    for index in lowIndex:
        ...
        cam1exc13.append(df.at[index,'CAM 1 EXC 1.3'])

    for index in TTLIndex:
        TTL1.append(df.at[index,'-.2'])
    ...
    for idm, mouse in enumerate(mice):
        if mouse != 'XX':
            ...
            mouseDat = {'Animal':mouseOut,'time':times,'TTL1':TTL1,'490nm':lowMouse}
...
def trial_sort(df):
    ...
    sessionStart = 307 # define time (s) where first trial begins
...
def trial_avg_plot(df, yvar, sessions, xvar ='Trial Time', trace='dFF', xlim=None, ylim=None):
    ...
    ylab = r'$\Delta F/F$ (%)' if trace is 'dFF' else r'Z-score'
    ...
    for session in sessions:
        ...
        ax.axvspan(0, 20, facecolor=jr_pal[7], alpha=0.2)
        if 'noShock' in session:
            pass
        else:
            ax.axvspan(26, 26.5, facecolor='black', alpha=0.2)
