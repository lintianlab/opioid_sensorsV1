...
def fp_trace(df, yvar='465nm', yiso='405nm',
    ...
    for idx in df['Animal'].unique():
        ...
        if trace is 'raw':
            ...
            ax = plt.plot(X, Y, color=kp_pal[4])
...
def tfc_trial_avg_old(df,
                      yvar='465nm_dFF_znorm',
                      xvar='time_trial',
                      cs_dur=20,
                      us_del=40,
                      us_dur=2,
                      fig_size=(12, 8), **kwargs):
    """
    ...
    mean_vals = (df.loc[:, ['Animal', 'Trial', xvar, yvar]]
                 .groupby([xvar]).mean().reset_index())
    # collapse across trials within each animal
    avg_subj_trial = (df.loc[:, ['Animal', 'Trial', xvar, yvar]]
                      .groupby(['Animal', xvar]).mean().reset_index())
    error_vals = avg_subj_trial.groupby([xvar]).std().reset_index()
